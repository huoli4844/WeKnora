...
| スキルカタログとサンドボックス | ワークスペースのスキルカタログ（ClawHub / SkillHub / git / zip）をセッション永続の Docker / E2B / Cube サンドボックスへインストール；`shell_exec`、ファイルツール、成果物、設定単位のネットワークポリシー；Local ホストプロセスバックエンドは削除；チャット横のインタラクティブターミナルとブラウザ内グラフィカルデスクトップ；macOS デスクトップ版はプロジェクトフォルダーに紐づく OS サンドボックス（Seatbelt）でセッションを実行 |
